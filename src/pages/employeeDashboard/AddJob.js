...
const AddJob = () => {
  const { companyName } = useSelector(state => state.authReducer.user)
  const { handleSubmit, register, control } = useForm({
    defaultValues: {
      companyName: companyName
    }
  });
  const [postJob, { isLoading, isError, data, error }] = usePostJobMutation();
  const {
    fields: resFields,
    append: resAppend,
    remove: resRemove,
  ...
  const onSubmit = (data) => {
    postJob({ ...data, jobState: true, applicants: [], queries: [], conversation: [] });
  };
